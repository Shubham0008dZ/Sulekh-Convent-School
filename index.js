/* ============================================================
   INDEX.JS — Homepage: all content from Google Sheets
   ============================================================ */
...
async function loadSettings() {
  try {
    ...
    const erpLink = document.getElementById('erpLoginLink');
    if (erpLink && data.erp_login_url) erpLink.href = data.erp_login_url;
  } catch(e) {}
}
...
async function loadSlider() {
  try {
    ...
    if (!slides || !slides.length) {
      // Show default placeholder slide
      track.innerHTML = `
        <div class="slide" style="background:var(--blue-dark);">
          <div class="slide-overlay"></div>
          <div class="slide-content">
            <h1 class="slide-title">Welcome to<br><span>Our School</span></h1>
            <p class="slide-sub">Add slides via CMS → Hero Slider</p>
          </div>
        </div>`;
      return;
    }
      ...
      <div class="slide" style="background:${s.bg_color || '#1a3a6b'};">
        ...
        <div class="slide-content">
          ...
          <div class="slide-actions">
            ...
            ${s.button2_text ? `<a href="${s.button2_link||'#'}" class="btn btn-outline"
              style="color:#fff;border-color:#fff;">${s.button2_text}</a>` : ''}
          </div>
        </div>
      </div>`).join('');
  } catch(e) { console.warn('Slider failed', e); }
}
...
async function loadNewsTicker() {
  try {
    ...
    track.innerHTML = items.map(n =>
      `<span class="ticker-item">${n.news_text}${n.link_url
        ? ` — <a href="${n.link_url}">Read More</a>` : ''}</span>`
    ).join('');
  } catch(e) {}
}
...
async function loadToppers() {
  try {
    ...
    if (!items || !items.length) {
      wrap.innerHTML = `<p style="color:var(--gray);text-align:center;padding:30px 0;font-size:14px;">
        No toppers added yet</p>`;
      return;
    }
      ...
      <div style="display:flex;align-items:center;gap:12px;
        ...
        <div>
          ...
          <div style="font-size:12px;color:var(--gray);">
            Class ${t.class} —
            <strong style="color:var(--red);">${t.percentage}%</strong>
            ${t.year ? ` (${t.year})` : ''}
          </div>
        </div>
      </div>`).join('');
...
async function loadBirthdays() {
  try {
    ...
    const items = await cmsGet('get_birthdays', { date });
    const wraps = document.querySelectorAll('.topper-placeholder');
    const wrap  = wraps[1];
    if (!wrap) return;

    if (!items || !items.length) {
      wrap.innerHTML = `<p style="color:var(--gray);text-align:center;
        padding:20px 0;font-size:14px;">No birthdays today 🎂</p>`;
      return;
    }
...
async function loadCirculars() {
  try {
    ...
    if (!items || !items.length) {
      wrap.innerHTML = `<p style="color:var(--gray);text-align:center;
        padding:30px 0;font-size:14px;">No circulars yet</p>`;
      return;
    }
        ...
        <div style="font-size:13px;font-weight:700;line-height:1.4;">
          ${c.file_url
            ? `<a href="${c.file_url}" target="_blank"
                 style="color:var(--blue-mid);text-decoration:none;">
                 📄 ${c.title}</a>`
            : `📄 ${c.title}`}
        </div>
        ${c.description
          ? `<div style="font-size:12px;color:var(--gray-dark);margin-top:3px;line-height:1.5;">
               ${c.description}</div>`
          : ''}
        ${c.date
          ? `<div style="font-size:11px;color:var(--gray);margin-top:3px;">${c.date}</div>`
          : ''}
      </div>`).join('');
  } catch(e) {}
}
...
async function loadDepartments() {
  try {
    ...
    if (!items || !items.length) {
      grid.innerHTML = `<p style="color:var(--gray);padding:40px;text-align:center;
        grid-column:1/-1;">No departments added yet. Add via CMS.</p>`;
      return;
    }

    const fallback = ['#1a3a6b', '#c8102e', '#1a6b3a', '#6b3a1a'];
    grid.innerHTML = items.map((d, i) => `
      <div class="dept-card animate-on-scroll">
        <div class="dept-img" style="background:${d.bg_color || fallback[i % 4]};">
          ${d.image_url
            ? `<img src="${d.image_url}" loading="lazy"
...
async function loadDirectorPrincipal() {
  try {
    ...
    setEl('directorQuote', dir.quote || dir.message);
    setEl('principalName', pri.name);
    setEl('principalQuote',pri.quote || pri.message);

    const avatars = document.querySelectorAll('.message-card .avatar-placeholder');
    if (dir.photo_url && avatars[0])
      avatars[0].innerHTML = `<img src="${dir.photo_url}" loading="lazy"
        style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="Director"/>`;
    if (pri.photo_url && avatars[1])
      avatars[1].innerHTML = `<img src="${pri.photo_url}" loading="lazy"
        style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="Principal"/>`;
  } catch(e) {}
}
...
async function loadGalleryPreview() {
  try {
    ...
    if (!items || !items.length) {
      grid.innerHTML = `<p style="color:var(--gray);padding:40px;text-align:center;
        grid-column:1/-1;">No gallery items yet. Add via CMS.</p>`;
      return;
    }
    ...
    grid.innerHTML = items.slice(0, 4).map(g => `
      <div class="gallery-item animate-on-scroll">
        <div class="gallery-img-placeholder"
          style="${g.thumbnail_url ? 'background:#000;padding:0;' : ''}">
          ${g.thumbnail_url
  ...
  try {
    ...
    track.innerHTML = items.map(p =>
      `<div class="partner-logo">
        ${p.logo_url
          ? `<img src="${p.logo_url}" loading="lazy" alt="${p.name}"
...
function initBirthdayDate() {
  ...
  const now = new Date();
  const mEl = document.getElementById('bdMonth');
  const dEl = document.getElementById('bdDay');
  if (mEl) mEl.textContent = months[now.getMonth()];
  if (dEl) dEl.textContent = String(now.getDate()).padStart(2, '0');
}

/* ── PARTNERS DUPLICATE for scroll ── */
function duplicatePartners() {
  const track = document.getElementById('partnersTrack') || document.querySelector('.partners-track');
  if (!track || track.dataset.dup || !track.children.length) return;
